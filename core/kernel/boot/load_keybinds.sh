...
# Common bindings
cat << EOF
bind-key -n $KEY_SWAP run-shell "$NEXUS_HOME/core/engine/lib/swap.sh"
bind-key -n $KEY_COMMAND command-prompt -p ":" "run-shell '$NEXUS_HOME/core/kernel/boot/dispatch.sh %%'"
EOF
