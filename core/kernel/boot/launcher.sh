...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --list)
            ...
            ls "$NEXUS_HOME/core/ui/compositions"/*.json | xargs -n 1 basename | sed 's/\.json//' | awk '{print "  - " $1}'
...
if [[ -f "$NEXUS_HOME/core/services/internal/hud_service.sh" ]]; then
    "$NEXUS_HOME/core/services/internal/hud_service.sh" start
fi

# Agent Follower Bridge (Ghosting)
if [[ "$NEXUS_FOLLOW_MODE" == "true" ]]; then
    echo "[*] Starting Agent Follower Bridge..."
    "$NEXUS_HOME/core/services/internal/follower_bridge.sh" &
...
if ! tmux has-session -t "$SESSION_ID:HUD" 2>/dev/null; then
    tmux new-window -d -t "$SESSION_ID:10" -n "HUD" -c "$PROJECT_ROOT" "$NEXUS_HOME/core/ui/hud/renderer.sh"
fi
...
if [[ -f "$NEXUS_HOME/core/engine/lib/workspace_manager.sh" ]]; then
