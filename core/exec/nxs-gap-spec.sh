...
if [[ "$PANE_TITLE" == "gap-spec" ]]; then
    ...
    # Spawn others
    $NEXUS_HOME/core/exec/nxs-tab.sh gap-spec new "$0 design"
    $NEXUS_HOME/core/exec/nxs-tab.sh gap-spec new "$0 tasks"
    $NEXUS_HOME/core/exec/nxs-tab.sh gap-spec new "$0 status"
